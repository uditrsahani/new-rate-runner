    ...
    try {
      await instance.get('/inquiry?minimize=true')
        .then((res) => {
          ...
          if (mounted.current) {
            setInquiryTables(res.data.filter((data) => (data.inq_status === 'waiting sales' || data.inq_status === 'waiting quotation') && data.inq_qtn !== 'SEND' && data.inq_disable !== 1).sort((a, b) => (a.inq_no.toUpperCase() < b.inq_no.toUpperCase() ? 1 : -1)));
            setInquiryList(res.data.filter((data) => (data.inq_status === 'waiting sales' || data.inq_status === 'waiting quotation') && data.inq_qtn !== 'SEND' && data.inq_disable !== 1)
