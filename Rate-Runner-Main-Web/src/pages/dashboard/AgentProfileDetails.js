...
import { AgentEdit } from '../../components/dashboard/AgentProfileDetails';
  ...
  useEffect(async () => {
    ...
    if (mounted) {
      await instance.get('/profile/Agent')
        .then((res) => {
          setAgent(res.data.filter((data) => data.agent_id === agentId));
        });
      await instance.get(`contact?ct_refer_id=${agentId}`)
        .then((res) => {
          setContact(res.data
