...
const InvoiceListTable = (props) => {
  ...
  const [page, setPage] = useState((menupage.recommend) ? menupage.recommend : 0);
  const [limit, setLimit] = useState((menupage.recommendL) ? menupage.recommendL : 10);

  const handleSelectAllInvoices = (event) => {
    setSelectedInvoices(event.target.checked
      ? InquiryTables.map((invoice) => invoice.id)
      : []);
  };
  ...
  const selectedSomeInvoices = selectedInvoices.length > 0
    && selectedInvoices.length < InquiryTables.length;
  const selectedAllInvoices = selectedInvoices.length === InquiryTables.length;
  return (
    <Card {...other}>
      <Grid
        container
                  ...
                  >
                    ...
                    <TableCell>
                      <IconButton
                        ...
                        to={`/main/Recommend/details/${invoice.inq_uuid}`}
                      >
