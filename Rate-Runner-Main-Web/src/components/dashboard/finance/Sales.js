...
const POL = (props) => {
  ...
  const getReport = useCallback(async () => {
    ...
    try {
      await instance.get(`/inquiry/report/top?group=sales&inq_status=close&inq_disable=0&inq_date_from=${start}&inq_date_to=${end}`)
        .then((res) => {
          const limit = 10;
          const tmpReport = [];
